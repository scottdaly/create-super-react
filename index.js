...
\`\`\`
${path.basename(root)}/
  frontend/  → Vite + React + TypeScript + Tailwind CSS v4${withAuth ? " + React Router (home/login/signup/dashboard) + CSRF helper + Google OAuth button" : ""}
  backend/   → Bun + Hono + SQLite (bun:sqlite)${withAuth ? " + local auth (secure cookies + scrypt + CSRF nonce + Origin check) + Google OAuth" : ""}
\`\`\`

### Frontend
- Vite React TS template
- Tailwind v4 via \`@tailwindcss/vite\` and \`@import "tailwindcss"\` in \`src/index.css\`.
${withAuth ? "- React Router routes: \`/\`, \`/login\`, \`/signup\`, \`/dashboard\` (protected)\n- \`apiFetch\` helper attaches CSRF headers to unsafe requests\n- Google button navigates to \`/api/auth/google/start\`\n- Vite dev proxy: \`/api\` → \`http://localhost:3000\`" : ""}
...
${withAuth ? `- Local auth:
  ...
  - CSRF: synchronizer token with short-lived nonce (10 min) + Origin check
  - Routes: POST /api/auth/signup, /login, /logout; GET /api/auth/session, /api/auth/csrf; GET /api/me
- Google OAuth (PKCE): GET /api/auth/google/start, GET /api/auth/google/callback
` : `- Example routes: GET /api/health; /api/todos (list/create)
`}
### Dev
