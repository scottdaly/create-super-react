...
async function writeClaudeMd(root, withAuth, withPasswordAuth) {
  const project = path.basename(root);

  if (withAuth) {
    if (withPasswordAuth) {
      // Full auth preset with password + Google
...
`;
    ...
    // Google-only auth preset (default)
    const content = `# CLAUDE.md
...
OAUTH_REDIRECT_URI=http://localhost:3000/api/auth/google/callback
\`\`\`
Then add the redirect URI in Google Cloud Console and restart the backend. Click **Sign in** to authenticate with Google.
...
function backendIndexWithPasswordAuthAndGoogle() {
  return `
import { Hono } from 'hono'
import { Database } from 'bun:sqlite'
...
function backendIndexGoogleOnly() {
  return `
import { Hono } from 'hono'
import { Database } from 'bun:sqlite'
import { setCookie, getCookie, deleteCookie } from 'hono/cookie'
...
// users (no password_hash for Google-only)
db.run(\`
CREATE TABLE IF NOT EXISTS users (
  id TEXT PRIMARY KEY,
  email TEXT UNIQUE NOT NULL,
...
function frontendFilesGoogleOnly() {
  return {
    "src/http.ts": `
...
function frontendFilesWithPasswordAuth() {
  return {
    "src/http.ts": `
type CsrfBundle = { nonce: string; token: string; exp: number }
let csrfCache: CsrfBundle | null = null
...
async function main() {
  ...
  const withPasswordAuth = rest.includes("--password-auth");
  const withAuth = !rest.includes("--minimal"); // default = Google auth
  ...
  if (withAuth) {
    if (withPasswordAuth) {
      ...
      const files = frontendFilesWithPasswordAuth();
      ...
      log("🧭 Adding React Router + Google OAuth auth...");
      run(`npm i react-router-dom`, { cwd: frontend });
      const files = frontendFilesGoogleOnly();
      for (const [rel, content] of Object.entries(files)) {
        const full = path.join(frontend, rel);
        await fs.mkdir(path.dirname(full), { recursive: true });
        await fs.writeFile(full, content);
  ...
  if (withAuth) {
    if (withPasswordAuth) {
      ...
      await fs.writeFile(path.join(backend, "src", "index.ts"), backendIndexWithPasswordAuthAndGoogle());
    } else {
      log("🔐 Writing Google OAuth only auth server...");
      await fs.writeFile(path.join(backend, "src", "index.ts"), backendIndexGoogleOnly());
    }
  ...
  await writeClaudeMd(root, withAuth, withPasswordAuth);
  ...
  if (withAuth) {
    if (withPasswordAuth) {
      log(`\nGoogle + Password auth enabled. Configure Google in backend/.env then try login/signup or "Continue with Google".`);
    } else {
      log(`\nGoogle-only auth enabled. Configure Google in backend/.env then try the "Continue with Google" button on /login.`);
    }
  } else {
